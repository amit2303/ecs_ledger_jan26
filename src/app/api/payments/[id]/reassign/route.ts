...
/**
 * PUT /api/payments/[id]/reassign
 * Body: { packageId: number }
 * 
 * Moves a payment from its current package to a different package.
 */
export async function PUT(request: Request, props: { params: Promise<{ id: string }> }) {
    try {
        ...
        // Get the current payment to find old package
        ...
        const oldPackageId = payment.packageId
        ...
        if (targetPackage.companyId !== payment.package.companyId) {
        ...
        return NextResponse.json({ message: 'Payment reassigned successfully' })
    } catch (error) {
        console.error('Payment Reassign Error:', error)
